...
(function ($) {
	$.scrollorama = function (options) {
		...
		function init() {
			...
			for (i = 0; i < scrollorama.settings.blocks.length; i++) {
				...
				blocks.push({
					...
					top:
						block.offset().top -
						(!Boolean(marginTop) ? parseInt(marginTop, 10) : 0),
					pin: 0,
					animations: [],
				});
			}
			...
			if (scrollorama.settings.enablePin.toString() === "true") {
				for (i = 0; i < blocks.length; i++) {
					blocks[i].block.css("position", "absolute").css("top", blocks[i].top);
				}
			}
			...
			for (i = 0; i < blocks.length; i++) {
				...
				if (blocks[i].animations.length) {
					for (j = 0; j < blocks[i].animations.length; j++) {
						...
						if (
							i === currBlockIndex ||
							(currBlockIndex === i - 1 && anim.baseline === "bottom")
						) {
							// if block gets pinned, set position fixed
							if (blocks[i].pin && currBlockIndex === i) {
								blocks[i].block.css("position", "fixed").css("top", 0);
							}
							...
							// otherwise, set value based on scroll
								...
								if (anim.easing && $.isFunction($.easing[anim.easing])) {
									animPercent = $.easing[anim.easing](
										animPercent,
										animPercent * 1000,
										0,
										1,
								...
								animVal =
									anim.startVal + animPercent * (anim.endVal - anim.startVal);
		...
		scrollorama.animate = function (target) {
			var targetIndex, targetBlock, anim, offset, suffix, i, j;
			/*
			...
			*/
			...
			for (i = 1; i < arguments.length; i++) {
				...
				if (anim.property === "rotate") {
					...
					anim.property === "letter-spacing" &&
					target.css(anim.property)
				) {
					if (anim.start === undefined) {
						anim.start = 1;
					}
				...
				if (
					ieVersion &&
					ieVersion < 9 &&
					(anim.property == "background-position-x" ||
						anim.property == "background-position-y")
				) {
				...
				startVal =
					anim.start !== undefined
						? typeof anim.start == "function"
							? anim.start()
							: anim.start
						: parseInt(target.css(anim.property), 10); // if undefined, use current css value
				endVal =
					anim.end !== undefined
						? typeof anim.end == "function"
							? anim.end()
							: anim.end
						: parseInt(target.css(anim.property), 10); // if undefined, use current css value
				suffix =
					startVal.toString().match(/\D+$/) || endVal.toString().match(/\D+$/);
				if (suffix) {
					suffix = suffix[0];
